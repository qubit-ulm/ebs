...
#ifndef BK_MAX_FLOW_H
#define BK_MAX_FLOW_H
...
protected:
    ...
    {
        auto res_cap = get(m_res_cap_map, f_edge_to_parent);
        BOOST_ASSERT(res_cap > 0);
...
bk_max_flow(Graph& g,
            ...
            typename boost::graph_traits<Graph>::vertex_descriptor sink)
{
    ...
    BOOST_CONCEPT_ASSERT(( boost::VertexListGraphConcept<Graph> )); //to have vertices(), num_vertices(),
    BOOST_CONCEPT_ASSERT(( boost::EdgeListGraphConcept<Graph> )); //to have edges()
    BOOST_CONCEPT_ASSERT(( boost::IncidenceGraphConcept<Graph> )); //to have source(), target() and out_edges()
    BOOST_CONCEPT_ASSERT(( boost::ReadablePropertyMapConcept<CapacityEdgeMap, edge_descriptor> )); //read flow-values from edges
    BOOST_CONCEPT_ASSERT(( boost::ReadWritePropertyMapConcept<ResidualCapacityEdgeMap, edge_descriptor> )); //write flow-values to residuals
    BOOST_CONCEPT_ASSERT(( boost::ReadablePropertyMapConcept<ReverseEdgeMap, edge_descriptor> )); //read out reverse edges
    BOOST_CONCEPT_ASSERT(( boost::ReadWritePropertyMapConcept<PredecessorMap, vertex_descriptor> )); //store predecessor there
    BOOST_CONCEPT_ASSERT(( boost::ReadWritePropertyMapConcept<ColorMap, vertex_descriptor> )); //write corresponding tree
    BOOST_CONCEPT_ASSERT(( boost::ReadWritePropertyMapConcept<DistanceMap, vertex_descriptor> )); //write distance to source/sink
    BOOST_CONCEPT_ASSERT(( boost::ReadablePropertyMapConcept<IndexMap, vertex_descriptor> )); //get index 0...|V|-1
...
bk_max_flow(Graph& g,
            ...
            typename boost::graph_traits<Graph>::vertex_descriptor sink)
{
    typename boost::graph_traits<Graph>::vertices_size_type n_verts = num_vertices(g);
    std::vector<typename boost::graph_traits<Graph>::edge_descriptor> predecessor_vec(n_verts);
    std::vector<typename boost::graph_traits<Graph>::vertices_size_type> distance_vec(n_verts);
    return
        bk_max_flow(
            g, cap, res_cap, rev,
